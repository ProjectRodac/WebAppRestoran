<?php
	...
	while(list($id, $naziv, $slika, $opis, $korisnik) = 
		...
		echo "<tr>
			...
			<td>$opis</td>
			<td>$korisnik</td>";
			if ($aktivni_korisnik_tip==0 or 1) {
				echo "<td><a href='statistika1.php?prostorija=$id' class='link'>Statistika</a></td>";
			}
			echo "</tr>";
	}
	echo "</table>";
	echo "<br/>";
	echo '<a class="link" href="statistika_meni.php">Povratak</a>';
	...
	if ($aktivna != 1) { 
		$prethodna = $aktivna - 1;
		echo "<a class='link' href=\"stat_prostorije.php?stranica=" .$prethodna . "\">&lt;</a>";	
	}
	for ($i = 1; $i <= $broj_stranica; $i++) {
		...
		echo "' href=\"stat_prostorije.php?stranica=" .$i . "\">$i</a>";
	}
	
	//ako je aktivna stranica zadnja, nije potrebno stavljati link za sljedecu stranicu
	if ($aktivna < $broj_stranica) {
		$sljedeca = $aktivna + 1;
		echo "<a class='link' href=\"stat_prostorije.php?stranica=" .$sljedeca . "\">&gt;</a>";	
	}
	echo '</div>';
